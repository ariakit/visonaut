...
After settlement, let the normal diagnostic scheduler deliver the five active current intents. Confirm each remote check is named `Visonaut`, has the intended status and conclusion, and has `delivered_revision=desired_revision` in D1. The inactive run has no current intent; resolve only its old `check-delivery` event through the existing guarded event helper after verifying revision 47 is obsolete. Remove the local `.dev.vars` file and stop Wrangler when finished.
