...
The pinned reusable workflow owns the complete matrix, test patterns, preview-server JSON (VISONAUT_WEB_SERVERS), and final visonaut submit --run job. Verify the npm tarball SHA-256 before extraction, copy ci/runtime-lock.json to ci/package-lock.json, then run npm ci in the package's ci directory. Render has no OIDC permission; only upload and final submit do.
`;
